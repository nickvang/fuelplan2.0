  ...
  return (
    <div className="container mx-auto px-4 py-6 sm:p-8 max-w-[1200px] min-w-0">
      <div className="mb-6 sm:mb-8">
        <h1 className="text-2xl sm:text-4xl font-bold mb-2">🔍 Algorithm Root Cause Analysis</h1>
        <p className="text-sm sm:text-base text-muted-foreground">
          Manual deep-dive into the hydration calculator logic without running full tests
        </p>
      </div>

      <Card className="p-4 sm:p-6 mb-6 bg-slate-50 dark:bg-slate-900 overflow-hidden">
        <pre className="text-sm whitespace-pre-wrap font-mono">{summary}</pre>
      </Card>

      <div className="space-y-6">
        {issues.map((issue, idx) => (
          <Card key={idx} className="p-4 sm:p-6 border-l-4 overflow-hidden" style={{
