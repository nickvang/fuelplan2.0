...
function formatDurationHHMMSS(hoursDecimal: number): string {
...
// Normalize distance labels: trim, collapse spaces, and lower-case.
const normalizeDistanceKey = (distance: string): string =>
  distance.trim().replace(/\s+/g, ' ').toLowerCase();

// Distance mapping in kilometers (all keys must be normalized with normalizeDistanceKey)
const raceDistanceMap: { [key: string]: number } = {
  ...
  // Triathlon distances (mapped to run leg distance for pace)
  'sprint': 5,
  'sprint triathlon': 5,
  'olympic': 10,
...
export function PaceDurationCalculator({
  ...
  // When goal time is set, don't let currentPace overwrite the calculated pace
  const hasGoalTime = Boolean(goalTime && raceDistance);
  useEffect(() => {
    if (currentPace && !hasGoalTime) {
      setInputValue(currentPace);
    }
  }, [currentPace, hasGoalTime]);

  const onPaceChangeRef = useRef(onPaceChange);
  const onDurationChangeRef = useRef(onDurationChange);
  onPaceChangeRef.current = onPaceChange;
  onDurationChangeRef.current = onDurationChange;

  // When goal time (or distance) changes, auto-calculate pace and duration and fill the pace field
  useEffect(() => {
    if (goalTime && raceDistance) {
      ...
      if (pace) {
        ...
        if (timeMatch) {
          ...
          setCalculatedValue(formatDurationHHMMSS(duration));
          onDurationChangeRef.current(duration);
          setFinishTime(goalTime);
        }
      }
    } else {
      setRequiredPace('');
    }
  }, [goalTime, raceDistance, discipline]);
  ...
  const calculatePaceFromGoalTime = (time: string, distance: string): string | null => {
    ...
    // Get distance (robust to spacing/casing)
    let distanceKm = raceDistanceMap[normalizeDistanceKey(distance)];
    if (!distanceKm) {
  ...
  const calculateDurationFromPace = (pace: string, distance?: string): number | null => {
    ...
    // Try to get distance from map (normalized), or parse as number
    let distanceKm = raceDistanceMap[normalizeDistanceKey(distance)];
    if (!distanceKm) {
  ...
  const handleInputChange = (value: string) => {
    ...
    if (raceDistance) {
      ...
      if (calculatedDuration !== null) {
        setCalculatedValue(formatDurationHHMMSS(calculatedDuration));
