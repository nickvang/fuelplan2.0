...
export async function parseSmartWatchFiles(files: File[]): Promise<{ 
  ...
  // Detect file type based on filenames (broad patterns to capture all exports)
  ...
  const isGarmin = fileNames.some(name => 
    ...
    name.includes('daily_summary') ||
    name.includes('activities') && name.endsWith('.csv') ||
  ...
  const tryParse = async (): Promise<{ profile: Partial<HydrationProfile>; rawData: ParsedGarminData | ParsedWhoopData | ParsedAppleWatchData | ParsedCorosData }> => {
    if (isWhoop) {
      const whoopData = await parseWhoopFiles(files);
      return { profile: mapWhoopDataToProfile(whoopData), rawData: whoopData };
    }
    if (isGarmin) {
      const garminData = await parseGarminFiles(files);
      return { profile: mapGarminDataToProfile(garminData), rawData: garminData };
    }
    if (isAppleWatch) {
      const appleWatchData = await parseAppleWatchFiles(files);
      return { profile: mapAppleWatchDataToProfile(appleWatchData), rawData: appleWatchData };
    }
    if (isCoros) {
      const corosData = await parseCorosFiles(files);
      return { profile: mapCorosDataToProfile(corosData), rawData: corosData };
    }
    return { profile: {}, rawData: {} };
  };

  try {
    let result = await tryParse();
    // If detection failed but we have CSV/FIT files, try Garmin then Whoop (common exports)
    const hasDataFiles = fileNames.some(n => n.endsWith('.csv') || n.endsWith('.fit'));
    if (hasDataFiles && Object.keys(result.profile).length === 0) {
      const garminData = await parseGarminFiles(files);
      const garminProfile = mapGarminDataToProfile(garminData);
      if (Object.keys(garminProfile).length > 0) {
        result = { profile: garminProfile, rawData: garminData };
      } else {
        const whoopData = await parseWhoopFiles(files);
        const whoopProfile = mapWhoopDataToProfile(whoopData);
        if (Object.keys(whoopProfile).length > 0) {
          result = { profile: whoopProfile, rawData: whoopData };
        }
      }
    }
    return result;
  } catch (err) {
    console.error('parseSmartWatchFiles error:', err);
    return { profile: {}, rawData: {} };
  }
}
